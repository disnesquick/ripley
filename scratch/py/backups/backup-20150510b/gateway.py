...
class Gateway:
	...
	def resolveTransverse(self, transverseID):
		...
		"""
		try:
			return self.router.getCachedTransverse(transverseID)
		except KeyError:
			objectID = (yield from self.transceiveResolve(transverseID))
			self.router.cacheTransverse(transverseID, objectID)
			return objectID
	...
	def serializeTransverseReference(self, transverse, outStream):
		...
		try:
			...
			ref = self.router.getCachedTransverse(transverseID)
			ObjectID.serialize(ref, outStream)
		except KeyError:
			...
			waiter = asyncio.async(self.router.waitForReply(messageID, self, reply))
	...
	@asyncio.coroutine
	def transceiveEval(self, callObject, readReply):
		...
		MessageID.serialize(messageID, outStream)
		self.serializeTransverseReference(callObject.iface, outStream)
		callObject.serializeArguments(self, outStream)
