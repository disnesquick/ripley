...
class RemoteCall:
	...
	def callOn(self, gateway):
		""" Sends the argument-bound call to a specific gateway for execution
		    on the remote end.
		"""
		objectID = yield from gateway.resolveTransverse(self.iface.__transverse_id__)
		return (yield from gateway.transceiveEval(objectID, self.serializeArguments, self.deserializeResponse))
...
class MethodProxy:
	...
	class Bound:
		...
		def __call__(self, *args, **kwargs):
			...
			remoteCall = RemoteCall(self.iface, (self.__self__,)+args, kwargs)
	...
	def __get__(self, instance, owner):
		return self.Bound(instance, self.iface)
