...
class BlackBox(PassByReference, metaclass = ABCMeta):
	""" Object reference with no interface.
	
	    This class is used when a caller wishes to pass a reference to a local
	    object, which may or may not have a transverse interface, for future
	    reference by an incoming call. The object has no interface and all the
	    remote end can do is include the object in a future call.
	"""
	@staticmethod
	def getProxyClass():
		return ProxyObject
